...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['addRecord'])) {
    ...
    foreach (PROJECTS as $projectKey => $projectInfo) {
        if (isset($_POST['project_' . $projectKey]) && $_POST['project_' . $projectKey] === 'on') {
            ...
            // 如果是优秀分享项目，记录分数
            if ($projectKey === 'excellent' && isset($_POST['excellent_score'])) {
...
<head>
    ...
    <style>
        ...
        .excellent-score { margin-left: 20px; display: none; }
    </style>
    <script>
        // 控制优秀分享分数选择的显示与隐藏
        document.addEventListener('DOMContentLoaded', function() {
            const excellentCheckbox = document.getElementById('project_excellent');
            const excellentScore = document.getElementById('excellent_score');
            
            if (excellentCheckbox && excellentScore) {
                // 初始状态检查
                toggleExcellentScore(excellentCheckbox.checked);
                
                // 监听复选框变化
                excellentCheckbox.addEventListener('change', function() {
                    toggleExcellentScore(this.checked);
                });
            }
            
            function toggleExcellentScore(show) {
                if (show) {
