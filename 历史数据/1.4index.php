...
// 优秀分享可选分数
define('EXCELLENT_SCORES', [90, 95, 100]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['addRecord'])) {
    ...
    // 如果是优秀分享项目，记录分数
    if ($_POST['project'] === 'excellent' && isset($_POST['excellent_score'])) {
        $newRecord['score'] = (int)$_POST['excellent_score'];
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['editRecord'])) {
    ...
    foreach ($data as &$record) {
        if ($record['id'] === $id) {
            $record['team'] = $_POST['team'];
            $record['name'] = $_POST['name'];
            $record['project'] = $_POST['project'];
            $record['date'] = $_POST['date'];
            
            // 如果是优秀分享项目，更新分数
            if ($_POST['project'] === 'excellent' && isset($_POST['excellent_score'])) {
                $record['score'] = (int)$_POST['excellent_score'];
            } elseif ($_POST['project'] !== 'excellent' && isset($record['score'])) {
                // 如果项目不是优秀分享，移除分数
                unset($record['score']);
            }
...
foreach ($teamMembers as $team => $members) {
    foreach ($members as $name) {
        ...
        $statistics[$team][$name] = [
            ...
            'excellentQuarter' => 0,
            'exerciseQuarter' => 0,
            'excellentWeekScores' => [], // 存储优秀分享的具体分数
            'excellentMonthScores' => [],
            'excellentQuarterScores' => []
        ];
        ...
        foreach ($allData as $record) {
            if ($record['name'] === $name && $record['team'] === $team) {
                ...
                // 获取项目分数，如果是优秀分享且有分数记录，使用记录的分数
                $score = ($project === 'excellent' && isset($record['score'])) 
                    ? $record['score'] 
                ...
                if ($date >= $currentWeekStart && $date <= $currentWeekEnd) {
                    ...
                    if ($project === 'excellent' && isset($record['score'])) {
                        $statistics[$team][$name]['excellentWeekScores'][] = $record['score'];
                    }
                }
                ...
                if ($date >= $currentMonthStart && $date <= $currentMonthEnd) {
                    ...
                    if ($project === 'excellent' && isset($record['score'])) {
                        $statistics[$team][$name]['excellentMonthScores'][] = $record['score'];
                    }
                }
                ...
                if ($date >= $currentQuarterStart && $date <= $currentQuarterEnd) {
                    ...
                    if ($project === 'excellent' && isset($record['score'])) {
                        $statistics[$team][$name]['excellentQuarterScores'][] = $record['score'];
                    }
                }
            }
        }
...
foreach ($statistics as $team => $members) {
    foreach ($members as $name => $stats) {
        // 检查是否有100分、95分、90分
        $has100 = in_array(100, $stats['excellentWeekScores']);
        $has95 = in_array(95, $stats['excellentWeekScores']);
        $has90 = in_array(90, $stats['excellentWeekScores']);
        ...
        // 月排名统计
        $has100Month = in_array(100, $stats['excellentMonthScores']);
        $has95Month = in_array(95, $stats['excellentMonthScores']);
        $has90Month = in_array(90, $stats['excellentMonthScores']);
...
<head>
    ...
    <style>
        ...
        .excellent-score { margin-left: 20px; }
    </style>
    <script>
        // 控制优秀分享分数选择的显示与隐藏
        document.addEventListener('DOMContentLoaded', function() {
            const projectSelect = document.getElementById('project');
            const excellentScore = document.getElementById('excellent_score');
            
            if (projectSelect && excellentScore) {
                // 初始状态检查
                toggleExcellentScore(projectSelect.value);
                
                // 监听项目选择变化
                projectSelect.addEventListener('change', function() {
                    toggleExcellentScore(this.value);
                });
            }
            
            function toggleExcellentScore(project) {
                if (project === 'excellent') {
                    excellentScore.style.display = 'inline-block';
                } else {
...
<body>
    ...
    <div class="container">
        <div class="main-content">
            <?php if ((isset($_GET['action']) && $_GET['action'] === 'add') || $editRecord): ?>
                <div class="table-container">
                    <h2><?php echo $editRecord ? '编辑记录' : '添加记录'; ?></h2>
                    <form method="post">
                        <?php if ($editRecord): ?>
                        ...
                        <div class="form-group">
                            ...
                            <select id="team" name="team" required>
                                ...
                                <?php foreach (TEAMS as $team): ?>
                                    <option value="<?php echo $team; ?>" <?php echo ($editRecord && $editRecord['team'] === $team) ? 'selected' : ''; ?>>
                                        <?php echo $team; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select id="project" name="project" required>
                                ...
                                <?php foreach (PROJECTS as $projectKey => $projectInfo): ?>
                                    <option value="<?php echo $projectKey; ?>" <?php echo ($editRecord && $editRecord['project'] === $projectKey) ? 'selected' : ''; ?>>
                                        <?php echo $projectInfo['name']; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            
                            <span class="excellent-score" id="excellent_score">
                                <label>分数:</label>
                                <select name="excellent_score">
                                    <?php foreach (EXCELLENT_SCORES as $score): ?>
                                        <option value="<?php echo $score; ?>" <?php echo ($editRecord && isset($editRecord['score']) && $editRecord['score'] === $score) ? 'selected' : ''; ?>>
                                            <?php echo $score; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" id="date" name="date" required
                                value="<?php echo $editRecord ? $editRecord['date'] : $currentDate; ?>">
                        </div>
                        <button type="submit" name="<?php echo $editRecord ? 'editRecord' : 'addRecord'; ?>" class="btn">
                            <?php echo $editRecord ? '保存修改' : '添加记录'; ?>
                        </button>
                    </form>
                </div>
            <?php else: ?>
                        ...
                        <tbody>
                            <?php foreach ($allData as $record): ?>
                                <tr>
                                    ...
                                    <td><?php echo ($record['project'] === 'excellent' && isset($record['score'])) ? $record['score'] : '-'; ?></td>
                        ...
                        <tbody>
                            <?php foreach ($excellentRankingWeek as $key => $item): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php 
                                        $scores = $statistics[$item['team']][$item['name']]['excellentWeekScores'];
                                        echo implode(', ', $scores);
                                        ?>
                                    </td>
                                </tr>
                        ...
                        <tbody>
                            <?php foreach ($excellentRankingMonth as $key => $item): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php 
                                        $scores = $statistics[$item['team']][$item['name']]['excellentMonthScores'];
                                        echo implode(', ', $scores);
                                        ?>
                                    </td>
                                </tr>
