...
<!-- 新增：本周数据统计 -->
<?php if (!empty($weeklyStats[$currentWeek])): ?>
    <h2>本周数据统计（<?php echo '第' . date('W', strtotime($currentWeek . '-01')) . '周，' . date('Y年m月d日', strtotime($currentWeek . '-01')) . '至' . date('Y年m月d日', strtotime($currentWeek . '-07')); ?>）</h2>
    <div class="stats-container">
        <?php foreach (['乐观组', '利他组'] as $team): ?>
            <?php if (isset($weeklyStats[$currentWeek][$team])): ?>
                <div class="stats-box">
                    <h3><?php echo $team; ?></h3>
                    <table>
                        <tr>
                        ...
                        <?php foreach ($weeklyStats[$currentWeek][$team] as $name => $stats): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($name); ?></td>
                                <td><?php echo $stats['reading']; ?></td>
                                <td><?php echo $stats['specialTotal']; ?></td>
...
<!-- 新增：本周优秀分享排名 -->
<?php if (!empty($weeklyRanking[$currentWeek])): ?>
    <div class="ranking">
        <h2>本周优秀分享学分排名（<?php echo '第' . date('W', strtotime($currentWeek . '-01')) . '周，' . date('Y年m月d日', strtotime($currentWeek . '-01')) . '至' . date('Y年m月d日', strtotime($currentWeek . '-07')); ?>）</h2>
        <?php foreach (['乐观组', '利他组'] as $team): ?>
            <?php if (isset($weeklyRanking[$currentWeek][$team])): ?>
                <h3><?php echo $team; ?></h3>
                <table>
                    <tr>
                        <th>排名</th>
                    ...
                    <?php foreach ($weeklyRanking[$currentWeek][$team] as $name => $ranking): ?>
                        <tr>
                            <td><?php echo $ranking['rank']; ?></td>
                            <td><?php echo htmlspecialchars($name); ?></td>
                            <td><?php echo $ranking['count']; ?></td>
