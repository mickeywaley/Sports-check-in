...
<body>
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            // 添加记录时检查当天是否已有同名记录（客户端预检查）
            if (this.getAttribute('name') === 'add') {
                const name = nameSelect.value || newName.value;
                const dateValue = date.value;
                
                // 检查最近20条记录中是否有重复（实际应用中可能需要AJAX验证）
                const recentRecords = document.querySelectorAll('table tr');
                for (let i = 1; i < recentRecords.length; i++) {
                    const cells = recentRecords[i].querySelectorAll('td');
                    if (cells.length >= 3) {
                        const recordDate = cells[0].textContent.split(' ')[0];
                        const recordName = cells[2].textContent;
                        
                        if (recordDate === dateValue && recordName === name) {
                            alert('当天已存在该用户的记录，不能重复添加');
                            e.preventDefault();
                            return false;
                        }
                    }
